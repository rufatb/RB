#!/usr/bin/env python3
"""Historical formatting helpers retained for compatibility with diagnostic callers.

The daily path is brief.compute -> renderers. No helper here acquires data.
"""
    ...
    for l in legs:
        ...
        if d <= 0:
            try:
                ...
                settled = (settled_out or {}).get(l["ticker"])
                if settled is None:
                    raise ValueError("resolution evidence was not supplied")
        ...
        try:
            ...
            out.append(f"      fundamentals unavailable ({type(exc).__name__})")
                ...
                for p_ in ([lg.get("pick") or {}]
                    ...
                    if p_.get("t"):
                        rows.append({"ticker": p_["t"],
                                     ...
                                     "price": p_.get("p945")})
            assessed = res.get("cost_assessed", [])
            L += _c.render(assessed, record=res.get("live_record") or {})
...
def render_record(rows: list, tides: dict | None = None) -> str:
    ...
    tides = {} if tides is None else tides
    if tides:
