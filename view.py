...
def _opportunities(d: dict) -> list:
    ...
    try:
        ...
        out += _wrap(f"approval-edge footnote unavailable "
                     f"({type(e).__name__})", 6, dim=True)
...
def _order_tail(d: dict, res: dict, side: str, p: dict) -> str:
    ...
    if p.get("shares") and not d.get("shadow"):
        try:
            ...
            # An order line without its fill bound is a different
            # instruction — say the bound is missing, never drop it silently.
            return (f"{p['shares']:>5} sh   {C.YELLOW}fill bound "
                    f"unavailable ({type(e).__name__}){C.RESET}")
...
def _pair_lines(d: dict, note: str) -> list:
    ...
    if known:
        ...
        try:
            ...
            # FAIL CLOSED. An unreadable clock means the spread MIGHT be a
            # stale post-close quote; the old code printed it as live, which
            # is the fail-open form of the same bug. Name the failed check.
            stale = True
            check_failed = type(e).__name__
        if stale and check_failed:
            out.append(f"      {C.YELLOW}spread not live — trading-hours "
                       f"check failed ({check_failed}){C.RESET}{C.DIM}; the "
                       f"~${total:,.0f} below may be{C.RESET}")
            out.append(f"      {C.DIM}a stale post-close bid/ask — verify the "
                       f"quote before acting on it.{C.RESET}")
        elif stale:
...
def _record(d: dict) -> list:
    ...
    try:
        ...
        out.append(f"    {C.DIM}advice record unavailable "
                   f"({type(e).__name__}){C.RESET}")
    return out
