...
def test_score_rows_direction_aware(tmp_path):
    ...
    rows = ledger.load(p)
    rows, n, _ = ledger.score_rows(rows, lambda t, d: {"A.TO": 101.0, "B.TO": 51.0}.get(t))
    assert n == 2
    ...
    rows, n2, _ = ledger.score_rows(rows, lambda t, d: 999.0)
...
def test_report_aggregates_by_tag(tmp_path):
    p = str(tmp_path / "l.csv")
    ledger.append_picks(_picks(), "2026-07-09", p)
    rows, _, _ = ledger.score_rows(ledger.load(p), lambda t, d: {"A.TO": 101.0, "B.TO": 51.0}.get(t))
